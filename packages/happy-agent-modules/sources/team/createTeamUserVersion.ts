...
/** Mint a UUIDv7 that is strictly newer than the team user version it replaces. */
export function createTeamUserVersion(previous?: string, now: () => number = Date.now): string {
...
function parseVersion(value: string): { readonly randomTail: bigint; readonly timestamp: bigint } {
    if (!UUID_V7_PATTERN.test(value)) {
        throw new Error("The stored team user version is invalid.");
    }
