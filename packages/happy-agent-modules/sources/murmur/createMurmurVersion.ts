...
/** Mint a UUIDv7 strictly newer than the durable sharing version it replaces. */
export function createMurmurVersion(previous?: string, now: () => number = Date.now): string {
...
function parseVersion(value: string): { readonly randomTail: bigint; readonly timestamp: bigint } {
    if (!UUID_V7_PATTERN.test(value)) {
        throw new Error("The stored sharing version is invalid.");
    }
