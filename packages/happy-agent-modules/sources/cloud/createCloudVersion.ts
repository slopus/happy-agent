...
/** Mint a Cloud UUIDv7 that is strictly newer than its prior public snapshot. */
export function createCloudVersion(previous?: string, now: () => number = Date.now): string {
...
function parseVersion(value: string): { readonly randomTail: bigint; readonly timestamp: bigint } {
    if (!UUID_V7_PATTERN.test(value)) throw new Error("The Cloud version is invalid.");
