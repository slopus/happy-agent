...
import { createHappyIntegrationDatabase } from "./HappyIntegrationDatabase.js";
import type {
...
export class HappyConnection implements HappySessionOperations, HappySpawnOperations {
    ...
    readonly #sync: ReturnType<typeof createHappySyncDatabase>;
    readonly #projectSync: ReturnType<typeof createHappyProjectSyncDatabase>;
    readonly #integrationDatabase: ReturnType<typeof createHappyIntegrationDatabase>;
    ...
    constructor(
        ...
        this.#sync = createHappySyncDatabase(owner?.id);
        this.#projectSync = createHappyProjectSyncDatabase(owner?.id);
        this.#integrationDatabase = createHappyIntegrationDatabase(owner?.id);
        this.#config = config;
    ...
    /** Reads this owner's Happy mobile integration snapshot. */
    async integration(_ctx: Context): Promise<HappyIntegration> {
    ...
    async #beginPairing(ctx: Context, generation: number): Promise<HappyIntegration> {
        ...
        return await this.#withLifecycleUpdate(async () => {
            ...
            let pairingContext = withAgentDatabase(detach(ctx).named("happy-pairing"), database);
    ...
    async #rememberUnlinkedCredentials(
        ...
        const external =
            this.#connectionOwner === undefined
                ? await readExternalHappyCredentialFingerprint({
                      environment: this.#config.happyEnvironment,
                  })
                : undefined;
    ...
    get #dataDirectory(): string {
        return this.#connectionOwner === undefined
            ...
            : this.#config.happyMobileDataDirectory(this.#connectionOwner.id);
    }
