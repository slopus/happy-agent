...
/** Mint a Happy integration UUIDv7 that is strictly newer than its prior snapshot. */
export function createHappyIntegrationVersion(
    previous?: string,
    now: () => number = Date.now,
): string {
...
function parseVersion(value: string): { readonly randomTail: bigint; readonly timestamp: bigint } {
    if (!UUID_V7_PATTERN.test(value)) {
        throw new Error("The Happy integration version is invalid.");
    }
