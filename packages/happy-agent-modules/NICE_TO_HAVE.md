...
the module cleanly with current public primitives, then keep the possible Base
improvement here for a separate task.

## Already covered by Agent Base 0.0.7
...
- caller-supplied message IDs and immutable message metadata on `send` and
  `steer`;
- awaited asynchronous `onEvent` and module observation hooks;
- agent metadata exposed to module scopes and transactional metadata-change
  hooks.

Modules should use these APIs directly. Do not add a second identity ledger or
heap-only agent/session link to recreate them.

## Host-level outer commit callbacks

Modules with application projections currently inject a store-specific
`afterCommit(ctx, callback)` contract. A first-class outermost-commit callback
on Agent Storage or Context would give every module one standard way to:
...
History, Collaboration, and future catalog modules.
...
Modules would still own their schemas and behavior. The helper would only
...
Agent Base 0.0.7 treats an omitted `serviceTier` (and the other optional
...
on loop, turn, inference, and settlement hook payloads would reduce module
...
Agent and message metadata intentionally allow module-owned JSON fields. Small
helpers for a module-owned namespace could prevent collisions and centralize
validation:

```ts
readModuleMetadata(schema, metadata, "collaboration");
writeModuleMetadata(metadata, "collaboration", value);
```

The underlying metadata should remain an ordinary persisted JSON object.

## Typed module configuration access

`AgentConfig.modules[name]` is correctly opaque to Agent Base. A scope helper
that accepts a module-owned TypeBox schema could remove repeated read/check
boilerplate while preserving that opacity:

```ts
scope.config(moduleName, schema);
```
...
Many modules need deterministic character, item, and page bounds for
model-facing text. A small Base utility package could standardize truncation
markers and UTF-safe limits without deciding module content or store paging.
