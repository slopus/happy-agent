...
The daemon serves HTTP/1.1 over a Unix domain socket. There is no TCP listener. The socket path
comes from the agent configuration (`paths.socketPath`); the socket file and its parent directory
are created with owner-only permissions (`0600` / `0700`).
...
The token is a 43-character base64url string generated by the daemon on first start and persisted
at `paths.tokenPath` with mode `0600`. A missing or mismatched token yields `401` with body
`{ "error": "Unauthorized" }`. There are no unauthenticated endpoints — health included.
...
One installation is one person, and the profile is what that person has chosen to say about
themselves: a name, an email, a photo. Every field is optional — the profile exists from the
first `GET`, empty, and fills in as the person sets things. It is a singleton at `/v0/profile`;
there is no list and no create.
