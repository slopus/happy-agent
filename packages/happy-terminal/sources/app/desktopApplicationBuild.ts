...
/** Builds and installs one current-architecture Happy app with a bundled Happy Terminal runtime. */
export async function desktopApplicationBuild(input: {
    ...
    const rigRuntime = join(stagingRoot, "happy-terminal-runtime");
    ...
    try {
        ...
        console.log("Building the current Happy Terminal runtime.");
        ...
        if (!stagedApplication) {
            throw new HappyTerminalUserError(
                "Happy finished packaging without producing a macOS app.",
            );
        }
        await commandRun(
            "/usr/bin/codesign",
        ...
        if (!application) {
            throw new HappyTerminalUserError(
                "The packaged Happy desktop app could not be installed.",
            );
        }
