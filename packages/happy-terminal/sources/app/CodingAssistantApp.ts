...
const INPUT_PLACEHOLDER = "Ask Happy Terminal to do anything";
const INPUT_PROMPT = "› ";
...
export class CodingAssistantApp implements Component, Focusable {
    ...
    async stop(reason: AppExitReason = "exit"): Promise<void> {
        ...
        try {
            ...
            // nothing Happy Terminal started outlives Happy Terminal.
        ...
        const details =
            debug === undefined
                ...
                : [
                      ...
                      {
                          label: "Live state",
                          reason: "Evaluate globalThis.__happyTerminalDebug in either inspector",
                      },
                  ];
        this.#appendEntry({
            noticeChildren: details,
            role: "event",
            text: "Open either inspector and evaluate globalThis.__happyTerminalDebug to walk the live process state.",
            title: "Debug",
        });
        this.#requestRender();
    }
    ...
    #cwdDisplayName(): string {
        const gymWorkspace = process.env.HAPPY_TERMINAL_GYM_DISPLAY_WORKSPACE?.trim();
