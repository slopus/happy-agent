...
export async function desktopApplicationContentHash(
    ...
    await repositoryFilesHash(hash, rigRoot, [
        ...
        "packages/happy-terminal/scripts",
        "packages/happy-terminal/sources",
...
export async function desktopApplicationResolve(
    ...
    for (const candidate of candidates) {
        ...
        const requiredPaths = [
            ...
            join(contents, "Resources", "happy-terminal-runtime", "node_modules"),
        ];
...
export async function rigRepositoryRootResolve(): Promise<string> {
    const configured = process.env.HAPPY_TERMINAL_SOURCE_DIRECTORY?.trim();
    const starts = [
    ...
    for (const start of starts) {
        ...
        if (root && (await pathExists(join(root, "packages", "happy-terminal", "package.json"))))
            return root;
    }
    throw new HappyTerminalUserError(
        "Happy Terminal desktop needs the Happy Terminal source checkout that built this command.",
        {
            hint: "Run the checkout's built CLI, or set HAPPY_TERMINAL_SOURCE_DIRECTORY to the Happy Terminal repository.",
        },
    );
}
...
export async function happy2RepositoryRootResolve(
    ...
    throw new HappyTerminalUserError(
        "Happy Terminal could not find a local Happy 2 source checkout.",
        {
            hint: "Run happy-terminal desktop --happy2-root /path/to/happy2.",
        },
    );
}
