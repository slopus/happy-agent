...
export function renderStartupStatusCard(options: {
    ...
    function statusRows(model: StartupStatusCardModel, width: number): string[] {
        if (width === 0) return [];
        const title = `${options.theme.brand}${BOLD}Happy Terminal${RESET} ${DIM}${model.version} · ${model.session}${RESET}`;
        const modelValues = [
            model.model,
            model.reasoning,
            model.provider,
        ...
        return [
            `${options.theme.brand}${BOLD}Happy Terminal${RESET} ${DIM}${model.version}${RESET}`,
