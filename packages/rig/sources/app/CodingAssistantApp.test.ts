...
describe("CodingAssistantApp", () => {
    ...
    it("shows a persistent above-composer indicator, in English, while a member holds a capability", () => {
        ...
        const rendered = stripAnsi(app.render(100).join("\n"));
        expect(rendered).toContain("2 members can watch a terminal in this session");
        // Never the protocol's capability code, only the English the share metadata carries.
        expect(rendered).not.toContain("terminal_view");
    });

    it("collapses the peer-capability indicator into history in the same render as the capability ending", () => {
        const model = defineModel({
            id: "openai/gpt-test",
            name: "GPT Test",
            thinkingLevels: ["off"],
        ...
        // A realistic transcript already has entries by the time a capability ends; an empty
        // one would hide a real regression behind an unrelated leading-blank-line difference.
        app.applySessionEvent({
            createdAt: 1,
            data: {
                delivery: "context",
                displayText: "Hello",
                message: { blocks: [{ text: "Hello", type: "text" }], id: "user-1", role: "user" },
                runId: "run-1",
            },
            id: "user-submitted",
            sessionId: "session-1",
        ...
        const activeFrame = app.render(100);
        const active = stripAnsi(activeFrame.join("\n"));
        expect(active).toContain("1 member can watch a terminal in this session");
        expect(active).not.toContain("No member can");
        ...
        const endedSession: ProtocolSession = {
            activity: { kind: "idle", label: "Idle", since: 0 },
            agent: { depth: 0, rootSessionId: "session-1", type: "primary" },
            agentId: snapshot.id,
            archived: false,
            ...
            secretIds: [],
            sessionSecretIds: [],
            shared: { ...sessionShare, capabilityMemberCount: 0 },
            snapshot,
            status: "idle",
            tasks: [],
            titleStatus: "idle",
        ...
        // The live row and the history row that explains its absence swap in one render, so
        // nothing above the composer collapses upward when the capability ends.
        app.applySessionEvent({
            createdAt: 1,
        ...
        const endedFrame = app.render(100);
        expect(endedFrame.length).toBe(activeFrame.length);
        const ended = stripAnsi(endedFrame.join("\n"));
        expect(ended).not.toContain("1 member can watch a terminal in this session");
        expect(ended).toContain("No member can watch a terminal in this session anymore.");
        expect(ended).not.toContain("terminal_view");
    });
});
