...
export async function ensureLocalProtocolServer(
    ...
    try {
        ...
        throw new RigUserError("The local Happy Agent daemon could not be started.", {
            cause: error,
...
export async function observeLocalProtocolServer(
    ...
    try {
        ...
        if (error instanceof RigUserError) throw error;
        return undefined;
    }
}
...
export function runDaemonInProcess(environment: NodeJS.ProcessEnv = process.env): boolean {
    return environment.RIG_GYM_IN_PROCESS_DAEMON === "1";
}
...
> {
    const override = process.env.RIG_GYM_HAPPY_AGENT_COMMAND?.trim();
    if (override !== undefined && override.length > 0) {
        let parsed: unknown;
        try {
            parsed = JSON.parse(override);
        } catch (error) {
            throw new Error("RIG_GYM_HAPPY_AGENT_COMMAND is not valid JSON.", { cause: error });
        }
        if (!Value.Check(commandSchema, parsed)) {
            throw new Error("RIG_GYM_HAPPY_AGENT_COMMAND must be a non-empty command array.");
        }
...
function assertCompatibleProtocol(health: HealthResponse): void {
    ...
    throw new RigUserError(
        `The running Happy Agent uses protocol ${String(health.version.protocol)}, but this Rig expects protocol ${String(HAPPY_AGENT_PROTOCOL_VERSION)}.`,
        { hint: "Stop the daemon and upgrade Rig before trying again." },
    );
}
