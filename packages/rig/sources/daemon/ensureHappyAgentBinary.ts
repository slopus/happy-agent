...
async function fetchLatestRelease(fetch_: typeof globalThis.fetch): Promise<Release> {
    const response = await fetch_(HAPPY_AGENT_LATEST_RELEASE_URL, {
        headers: {
            ...
            "user-agent": "Rig Happy Agent downloader",
...
async function downloadArchive(
    ...
    const response = await fetch_(asset.browser_download_url, {
        headers: { accept: "application/octet-stream", "user-agent": "Rig Happy Agent downloader" },
...
async function acquireInstallLock(
    ...
    for (;;) {
        ...
        if (!announcedWait) {
            announcedWait = true;
            onStatus?.("Waiting for another Rig process to finish downloading Happy Agent.");
        }
        ...
        if (Date.now() >= deadline) {
            throw new Error("Timed out waiting for another Rig process to install Happy Agent.");
        }
