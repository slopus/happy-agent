...
/**
 * Refuses a release that would leave `0.x` before Happy Terminal is ready to promise it.
 *
 * A major version is a promise about compatibility, and Happy Terminal is early enough to break its own
...
export function assertReleaseBumpAllowed({ currentVersion, requested }: ReleaseBumpInput): void {
    ...
    if (requested === "major" || requested === "premajor") {
        throw new Error(
            `Happy Terminal is still on ${currentVersion}, so a '${requested}' release is not available. ` +
    ...
    if (Number.isInteger(requestedMajor) && requestedMajor > 0) {
        throw new Error(
            `Happy Terminal is still on ${currentVersion}, so it does not release ${requested} yet. ` +
                `Use 'minor' for new features and 'patch' for fixes.`,
        );
    }
}
